(function($, jQuery) {
  $(document).ready(function() {
    console.log('bb1'); // which version
    // if back button, reload the page
      ...
      await $.getJSON(encodeURI(json_url), function(json1) {
        ...
        for (var p = 0; p <= existingStarts_start.length; p++) {
          ...
          if (my_overlap) {
            ...
            // next check if the user exceeds the 1 per 28 days limitation
            //$('.webform-submission-form').submit();
            //console.log('next check if the user exceeds the 1 per 28 days limitation');
            checkMonthlyLimit();
          }
    ...
    // verify the json
    // change the behavior to disable the form, change the button label
    // test it with Admin, Anon
    // clean up, document it
    async function checkMonthlyLimit(text) {
      ...
      var mini333 = dayjs(date1.val()).subtract(28, 'days').format('YYYY-MM-DD');
      var maxi444 = dayjs(date1.val()).add(28, 'days').format('YYYY-MM-DD');
      var email = $('#edit-group-representative-email-address').val();
      var json_rules = '/mr_rules.json?sid=' + sid + '&loc=' + loc + '&t=' + Date.now() + '&date333=' + mini333 + '&date444=' + maxi444 + '&email=' + email;
      //console.log(json_rules);
      await $.getJSON(encodeURI(json_rules), function(json333) {
        if (json333.length > 0) {
          $('body').addClass('sooner');
          $('#edit-submit').val('Only one reservation per branch within 28 days');
        } else {
          //   console.log('no 28 day conflicts');
          $('.webform-submission-form').submit();
        }
      });
    }
    ...
    var disabledDays = ['2022-11-11', '2022-11-24', '2022-11-25', '2022-12-23', '2022-12-24', '2022-12-25', '2022-12-26'];
    var max_date = dayjs().add(28, 'days');
    var min_date = dayjs();
    var which_date = $('#edit-date');
    var how_many = $('#edit-location').val();
    var location2 = $('#edit-location').val();
    var loc = $('#edit-location').val();
    var opening_hour = 9;
    var closing_hour = 20;
    ...
    $.getJSON("/roles.json?t=" + Date.now(), function(json9) {
      ...
      if (is_admin != 1 && is_internal != 1) {
        ...
        // remove some rooms that are not currently available
        // hide University Hills Branch if Anonymous
        //  $('option[value="185"]').css('display', 'none');
        // hide Cepeda Branch if Anonymous
        // $('option[value="186"]').css('display', 'none');
      }
  ...
  // future enhancement: look up holidays and other closures dynamically
  function get_special_dates() {
    disabledDays = [];
    $.getJSON("/special_dates3.json", function(json) {
      var k;
...
*/
      ...
      async function get_room(location2) {
        ...
        switch (location2) {
          ...
          case "3":
            ...
            break;
          case "205":
            rooms_array = [793, 795, 794];
            rooms_names = ['Ruiz Branch #1', 'Ruiz Branch #2', 'Ruiz Branch #1 & #2'];
            rooms_capac = [39, 39, 78];
            break;
          case "207":
      ...
      function check_dates(input_date, room_id) {
        ...
        var messages = [];
        //  Austin History Center 780, Manchaca Road 787, Little Walnut 786, Ruiz 793,795,794 and Spicewood Springs 799
        //  open 12-5 Sundays, beginning Sep11
        const arr = ['780', '787', '786', '793', '795', '794', '799'];
        const notIncludes = !arr.includes(room_id);
        if (ch_date.format('d') == 0 && notIncludes) {
          messages = ['This location is closed on Sunday.'];
        } else { // not a Sunday, or not a branch that's closed on Sunday
          if ((room_id == '780' || room_id == '7') && (parseInt(ch_date.format('d')) == '1')) { //closed on Monday if AHC 
            messages = ['This location is closed on Monday.'];
          } else {
            //  Austin History Center 780, Manchaca Road 787, Little Walnut 786, Ruiz 793,795,794 and Spicewood Springs 799
            //  open 12-5 Sundays
...
//        loop through options for duration, getting rid of options exceeding number of minutes 
      ...
      if (e_date.val() && e_time.val()) {
        ...
        avail_wrapper.prepend('<a class="e_date pp_avail active" data-time="' + e_time.val() + '" data-time2="' + e_time2.val() + '" data-date="' + e_date.val() + '">' + ee_date.format(
          f_time) + ' to ' + ee_date2 + '</a><br>');
        submit_button.prop('disabled', false);
      }
      var room_id = $('#edit-meeting-room').val();
      ...
      which_date.change(function(e) {
       // console.log(location1.val());
        var my_date = dayjs(which_date.val());
        var the_date1 = dayjs(which_date.val());
        var ppwf_format = 'YYYY-MM-DD';
        var the_date = the_date1.format(ppwf_format);
        if (the_date == '2022-07-04' || the_date == '2022-09-04' || the_date == '2022-09-05' || the_date == '2022-11-11' || the_date == '2022-11-24' || the_date == '2022-11-25' ||
          the_date == '2022-12-23' || the_date == '2022-12-24' || the_date == '2022-12-25' || the_date == '2022-12-26') {
          alert("We're closed on holidays. Please select another date.");
          which_date.val('');
        } else {
          if ((my_date > max_date) && (is_admin != '1' && is_internal != '1')) {
            var lib_date = dayjs().add(1, 'day').format(lib_format);
            which_date.val(lib_date);
          } else {
            if ($('#edit-location').val()) {
              if (dayjs(which_date.val()).isValid()) {
                get_room(location1.val());
